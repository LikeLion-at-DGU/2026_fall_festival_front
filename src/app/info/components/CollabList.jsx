...
export default function CollabList({ collabs = [], onSelect }) {
  ...
  return <S.Stack>
    ...
    <S.List>{visibleCollabs.map((item) => <S.Card key={item.id} type="button" onClick={() => onSelect(item.id)}><S.Thumbnail>{item.imageUrl && <img src={item.imageUrl} alt="" />}</S.Thumbnail><S.Body><strong>{item.name}</strong><span>{item.description}</span></S.Body><S.Chevron>›</S.Chevron></S.Card>)}</S.List>
